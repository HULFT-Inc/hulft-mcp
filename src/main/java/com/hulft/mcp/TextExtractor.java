...
public class TextExtractor {
    ...
    public TextExtractor(software.amazon.awssdk.services.textract.TextractClient textractClient) {
        this.textractClient = textractClient;
    }
    
    public String extractFromPdf(byte[] fileBytes, String filename) {
        return analyzeWithTextract(fileBytes, filename);
    }
    
    public String extractFromImage(byte[] fileBytes, String filename) {
        return analyzeWithTextract(fileBytes, filename);
    }
    
    public String extractFromExcel(byte[] fileBytes) {
        try {
    ...
    public Float getLastOcrConfidence() {
        Float conf = ocrConfidence.get();
        ocrConfidence.remove();
        return conf;
    }
    
    @SuppressWarnings("PMD.AvoidCatchingGenericException")
    private String analyzeWithTextract(byte[] fileBytes, String filename) {
        try {
